...
fn jacobi_decomposition<const R: usize>(
    ...
    for _ in 0..kmax {
        let (max, index) = max_lower_element(a_mat);
        if max < tol {
            break;
        }
...
mod tests {
    ...
    fn test_svd() {
        ...
        let (mut calc_eigv_a, _) = jacobi_decomposition::<3>(&mat_a, 0.00001, 1000);
        calc_eigv_a
            .as_mut_slice()
            .sort_by(|a, b| b.partial_cmp(a).unwrap());
        println!("{}", calc_eigv_a);
        assert!((eig_value_a - calc_eigv_a).apply_norm(&UniformNorm) < 0.001);
        ...
        assert!((eig_value_b - calc_eigv_b).apply_norm(&UniformNorm) < 0.001);
    }
}
